...
class CrfNegA(CrfNeg):
    def __init__(
        ...
        super(CrfNegA, self).__init__(V, L, A, S, emb_sz, rnn_sz, nlayers, dp)
        ...
        if self.outer_plate:
            ...
            self.proj_neg = nn.Linear(emb_sz, 2)
    ...
    def forward(self, x, lens, a, l):
        ...
        if self.outer_plate:
            ...
            # attend
            un_attn = torch.einsum("ntj,nsi->nts", [x, x])
            # attention dist is last dim
            un_attn.masked_fill_(mask.unsqueeze(1), float("-inf"))
            attn = un_attn.softmax(-1)
            neg_x = torch.einsum("nts,nsh->nth", [attn, emb_neg])
            phi_neg = self.proj_neg(neg_x)
        # CONV
        if self.outer_plate:
        ...
        psi_ybs.masked_fill_(mask.view(N, T, 1, 1, 1), 0)
        #import pdb; pdb.set_trace()
        ...
        if self.training:
            self._N += 1
        if self._N > 300 and self.training:
            # marginal density b
            phi_yl, phi_sl, phi_negl, phi_bl, psi_bbl, psi_ybsl = self.potentials(
                phi_s, phi_neg, phi_b, psi_bb, psi_ybs)
            hs, hb = (
                torch.stack(l, 1)
                for l in self.marginals(phi_yl, phi_sl, phi_negl, phi_bl, psi_bbl, psi_ybsl)
            )
            ps = hs.softmax(-1)
            pb = hb.softmax(-1)
